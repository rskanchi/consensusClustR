...
performConsensusClustering(
  exprFile, # expression data file with sample rows and feature columns, .csv file
  exprRowsSamples = TRUE, # if expression data rows are features, change to FALSE
  outputFolder = "output", # output folder name
  traitFile = NULL, # optional traits/meta/outcome data, for heatmap top annotation, rows as samples, .csv file
  ...
  nVarexpr = NULL, # optional number of highly variable expression data features to include ex. 2000
  ...
  nReps = 500, # number of cross-validation iterations
  ...
  expr.dist.method = "spearman", # "pearson" = (1-Pearson cor), "spearman" (1-Spearman cor), "euclidean", "binary", "maximum", "canberra", "minkowski" or custom distance function
